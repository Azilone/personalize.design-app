...
- Decision: bill the merchant only when an image is successfully produced _and stored_ (i.e., when provider cost has been incurred and output is persisted).
- Mechanism:
...
- Pattern: **Modal/Portal Architecture.**
  - Desktop: Centered Modal (Dialog).
  - Mobile: Full-screen Drawer/View.
  - Mount point: `document.body` (via React Portal) to ensure proper layering over theme elements.
  - Trigger: "Personalize & Order" button replaces/augments standard "Add to Cart".

**Client State Management:**

- Decision: Zustand (storefront only) for stepper state (Input -> Generating -> Raw Result -> Mockup Preview).

**Async Mockup Flow (Smart Polling):**

- **Trigger:** Frontend calls `POST /api/app-proxy/generate`.
- **Backend:** Starts Inngest workflow (Generate -> Bill -> Store). Returns `job_id` immediately (or after short wait if fast).
- **Mockup Optimization:** The Inngest workflow _automatically_ triggers the Mockup Generation step immediately after the raw image is ready. It does _not_ wait for a second user action. This "prefetching" ensures that by the time the user clicks "Preview", the mockups are likely already done or processing.
- **Frontend:**
  - Displays "Generating..." state.
  - Polls `GET /api/app-proxy/status/:job_id` (interval: 2s) to get the Raw Image.
  - Once Raw Image is shown, continues polling (background) for Mockup URLs.
  - If user clicks "Preview" before mockups are ready, show specific "Preparing mockups..." state.
