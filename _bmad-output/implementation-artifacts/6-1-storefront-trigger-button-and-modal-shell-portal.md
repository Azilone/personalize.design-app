...
- Updated `useStepperStore` to handle `isOpen` state and configuration context (`templateId`, `personalizationEnabled`).
- Updated `personalize_stepper.liquid` to expose `product.metafields.personalize_design.config` to the React bundle via data attributes.
- Added `useMediaQuery` hook for responsive behavior (Desktop vs Mobile).
- Added tests for store logic.
- Verified compilation and type checking.
- Updated `product-template-assignment.server.ts` and `app/routes/app/products/$productId/route.tsx` to sync template assignment to Shopify Metafields (missing link for storefront).
- Deployed changes to dev store.
- Verified live code availability via Chrome DevTools.
